...
// QuantizedDepthwiseConvKernel<...> specializations in
// depthwiseconv_uint8.h (the legacy uint8 per-tensor quant path).
//
// Math: per output pixel, per input channel c:
//   i16 input_val  = zero_extend(input[c]) + input_offset    // signed add
//   i16 filter_val = zero_extend(filter[c*M+k]) + filter_offset
//   acc[c*M + k] += (i32)(input_val * filter_val)
//
// Spec: INT8 algo-level error <= 1 LSB; we want integer-exact (== 0).
...
#include "tflite/kernels/internal/optimized/depthwiseconv_uint8.h"
...
void ScalarReference(int num_output_pixels, int input_depth,
  ...
  for (int outp = 0; outp < num_output_pixels; outp++) {
    const uint8_t* local_filter = filter_ptr;
    const uint8_t* local_input = input_ptr;
    for (int ic = 0; ic < input_depth; ic++) {
      ...
      for (int k = 0; k < depth_multiplier; k++) {
        const int16_t filter_val =
            static_cast<int16_t>(*local_filter++) + filter_offset;
        *acc_buffer_ptr++ +=
...
void RunKernel(const Case& c, const uint8_t* input, int16_t input_offset,
               int input_ptr_increment, const uint8_t* filter,
               int16_t filter_offset, int32_t* acc) {
  using tflite::optimized_ops::depthwise_conv::QuantizedDepthwiseConvKernel;
  switch (c.depth_multiplier) {
    case 1:
      QuantizedDepthwiseConvKernel<true, 0, 1>::Run(
          c.num_output_pixels, c.input_depth, 1, input, input_offset,
          input_ptr_increment, filter, filter_offset, acc);
      break;
    case 2:
...
bool RunCase(const Case& c, std::mt19937& rng) {
  // Realistic uint8 quant ranges: values [0, 255]; offsets typically in
  // [-255, 0] or [-128, 128] depending on (zero_point, range).
  std::uniform_int_distribution<int> dist8(0, 255);
  std::uniform_int_distribution<int> dist_off(-255, 255);
  ...
  const int16_t filter_offset = static_cast<int16_t>(dist_off(rng));

  std::vector<uint8_t> input(static_cast<size_t>(c.num_output_pixels) *
                                 c.input_depth +
                             input_ptr_increment);
  std::vector<uint8_t> filter(static_cast<size_t>(c.input_depth) *
                              c.depth_multiplier);
  std::vector<int32_t> acc_init(static_cast<size_t>(c.num_output_pixels) *
                                output_depth);
  for (auto& v : input) v = static_cast<uint8_t>(dist8(rng));
  for (auto& v : filter) v = static_cast<uint8_t>(dist8(rng));
  ...
  std::printf(
      "  %-12s mult=%-2d n_out=%2d depth=%4d in_off=%+4d fi_off=%+4d  "
      "max_abs_diff=%lld  %s\n",
      c.name, c.depth_multiplier, c.num_output_pixels, c.input_depth,
      input_offset, filter_offset, static_cast<long long>(max_abs_diff),
      ok ? "PASS" : "FAIL");
  return ok;
}
...
int main(int argc, char** argv) {
  ...
  std::printf(
      "RVSPOC S2601 — RVV legacy uint8 depthwise QuantizedDepthwiseConvKernel "
